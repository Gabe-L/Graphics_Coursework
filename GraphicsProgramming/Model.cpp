...
bool Model::loadModel(char* filename)
{
	...
	for (int i = 0; i < faces.size(); i+=3) {
		Vector3 tempVert, tempNorm, tempTex;
		tempVert = verts[(faces[i]-1)];
		tempTex = texCs[(faces[i + 1]-1)];
		tempNorm = norms[(faces[i + 2]-1)];

		vertex.push_back(tempVert.x); vertex.push_back(tempVert.y); vertex.push_back(tempVert.z);
		texCoords.push_back(tempTex.x); texCoords.push_back(tempTex.y);
		normals.push_back(tempNorm.x); normals.push_back(tempNorm.y); normals.push_back(tempNorm.z);
	}
